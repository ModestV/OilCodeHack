"""Runtime adapter for the repaired two-stage sulphur forecast (v4r, artifact version 5).

Opt-in only: ``backend.forecast.forecast_sulfur`` serves the v3 model unless
``OILCODE_FORECAST_MODEL=two-stage-v4r`` is set.  The output keeps the v3
contract used by the scenario, quality and blend agents (``status``,
``nowcast``, ``horizons``, ``path_supported``, interval and exceedance
probability at one terminal level), so the process chain (batch arrival,
stock, transport delay, blend and additive balance) is not replaced.

Features are rebuilt from the imported Parquet dataset with the offline code
(``tools/modeling/two_stage_features.py``).  The artifact holds one model set
per ``fit_end``; the latest set whose ``fit_end`` is not after the origin is
used, so a historical origin never sees a model fitted on later labels.
"""
...
from .forecast import ForecastUnavailable

ARTIFACT = ROOT / "reports" / "modeling" / "two-stage-v4r" / "model.json"
ARTIFACT_VERSION = 5
...
def _validate_artifact(artifact: dict[str, Any]) -> None:
    ...
    if artifact.get("version") != ARTIFACT_VERSION or any(key not in artifact for key in required):
        raise ForecastUnavailable("Артефакт двухступенчатой модели имеет неизвестный временной контракт")
    ...
    for entry in entries:
        if not entry.get("deployable"):
            raise ForecastUnavailable("Walk-forward модель без out-of-fold квантилей не может обслуживать прогноз")
        for horizon in horizons:
            ...
            for model in (stage2, entry.get("persistence", {}).get(key)):
...
def load_artifact(path: Path = ARTIFACT) -> dict[str, Any]:
    ...
    try:
        ...
        raise ForecastUnavailable("Артефакт двухступенчатой модели прогноза серы отсутствует или повреждён") from exc
...
def _stage1(entry: dict, horizon: int, features: pd.Series, policy: dict) -> tuple[dict[str, dict], bool]:
    """Anchored Stage-1 estimates per analyser at origin+horizon and whether any fell back to persistence."""
    raw = features.loc[DYNAMICS_COLUMNS].to_numpy(dtype=float)
    missing_fraction = float((~np.isfinite(raw)).mean())
    out, any_fallback = {}, False
    for name in ANALYSER_METRICS:
        model = entry["stage1"][name][str(horizon)]
...
def _horizon_estimate(entry: dict, horizon: int, features: pd.Series, artifact: dict) -> dict:
    key = str(horizon)
    stage2 = entry["stage2"][key]
    stage1, fallback = {}, False
    if horizon == 0:
        inputs = {"ln_q21": features["ln_q21"], "ln_pak": features["ln_pak"]}
    else:
        stage1, fallback = _stage1(entry, horizon, features, artifact["applicability_policy"])
    ...
    model = entry["persistence"][key] if fallback else stage2
    if fallback:
        for name in ANALYSER_METRICS:
    ...
    return {"ln_prediction": float(stage2_predict(model, vector)), "stage1": stage1, "fallback": fallback,
...
def _bracket(horizons: list[int], horizon: float) -> tuple[int, int, float]:
    ...
    return lower, upper, 0.0 if lower == upper else (horizon - lower) / (upper - lower)
...
def exceedance_at(entry: dict, horizon: float, ln_prediction: float, *, persistence: bool = False,
                  limit: float = 10.0) -> dict:
    """P(actual > limit) and 80% interval around one ln level (the same level as the point forecast)."""
...
def forecast_sulfur(directory: Path, at: str, artifact_path: Path = ARTIFACT, horizon_minutes: float = MAX_HORIZON_MINUTES) -> dict[str, Any]:
    ...
    horizons = [int(h) for h in artifact["horizons_minutes"]]
    entry = select_model(artifact, origin)

    per_horizon: dict[int, dict] = {}
    for h in horizons:
        if entry is None:
            gate = {"status": "abstain", "reasons": ["model_not_yet_available_at_origin"],
                    "missing_fraction": float((~np.isfinite(raw)).mean()), "policy": artifact["applicability_policy"]}
            per_horizon[h] = {"support": gate}
            continue
        gate = applicability(raw, {**entry["support"][str(h)], "applicability_policy": artifact["applicability_policy"]}, anchor_pairs)
        item = {"support": gate}
        if gate["status"] == "ok":
            estimate = _horizon_estimate(entry, h, features, artifact)
    ...
    path_supported = support["status"] == "ok" and all(per_horizon[h]["support"]["status"] == "ok" for h in horizons if h <= upper_h)

    def result_for(h: int) -> dict | None:
        item = per_horizon[h]
        if item["support"]["status"] != "ok" or "ln_prediction" not in item:
            return None
        return exceedance_at(entry, h, item["ln_prediction"], persistence=item["fallback"], limit=hard_limit)

    nowcast = result_for(0)
    horizon_result = None
    if support["status"] == "ok":
        a, b = per_horizon[lower_h], per_horizon[upper_h]
        ln_pred = (1 - weight) * a["ln_prediction"] + weight * b["ln_prediction"]
        horizon_result = exceedance_at(entry, horizon, ln_pred, persistence=a["fallback"] or b["fallback"], limit=hard_limit)
    horizon_rows = []
    for h in horizons:
        item = per_horizon[h]
        row = {"minutes": h, "status": item["support"]["status"], "reasons": item["support"]["reasons"]}
        res = result_for(h)
        if res:
            row.update(res)
            row.update({"stage1": item["stage1"], "path": item["path"], "stage2_kind": item["kind"], "inputs": item["inputs"]})
        else:
            row.update({"prediction": None, "lower": None, "upper": None, "exceedance_probability": None})
        horizon_rows.append(row)
    nearest = min(horizons, key=lambda h: abs(h - horizon))
    alarm_probability = float(entry["stage2"][str(nearest)]["alarm_probability"]) if entry else None
    stage1_status = None
    if entry and upper_h and "stage1" in per_horizon[upper_h]:
        stage1_status = "fallback_persistence" if per_horizon[upper_h]["fallback"] else "ok"
    ...
    warnings.append("Исследовательская двухступенчатая модель (v4r) в теневом режиме: не заменяет v3 без слепой проверки.")
    ...
    return {
        "at": origin.isoformat(), "prediction_origin": origin.isoformat(), "target_time": (origin + timedelta(minutes=horizon)).isoformat(),
        ...
        "hard_limit": hard_limit, "nowcast": nowcast,
        "nowcast_applicability": per_horizon[0]["support"], "path_supported": path_supported,
        ...
        "in_flight_controls": {c: (float(features[c]) if pd.notna(features[c]) else None) for c in CONTROL_DELTA_COLUMNS},
        ...
        "model": {"name": artifact.get("model"), "family": "two-stage-v4r", "horizons_minutes": horizons, "artifact": artifact_path.name,
                  "artifact_sha256": artifact["sha256"], "fit_end": entry["fit_end"] if entry else None,
                  "stage2_kind_by_horizon": {h: (entry["stage2"][str(h)].get("label") or entry["stage2"][str(h)].get("kind")) for h in horizons} if entry else None},
