...
- POST /api/datasets/{id}/scenario accepts editable source sulfur, targets, horizon 0–180 min, step 15–60 min, linear model parameters, optional control changes, reservoir components and additive 0–3%. Returns the three assumed controls, sulfur trajectory, blend quality, cost index and explicit assumptions.
- POST /api/datasets/{id}/decision runs the local QualityAgent → ReliabilityAgent → OptimizationAgent → Orchestrator contour with the same request body. It returns an explainable trace, three deterministic candidates (`hold`, `conservative`, `automatic`), a safety gate/ranking result and either a scenario recommendation or `abstain` with a reason when quality/freshness/control evidence is insufficient. A partial control vector is treated as insufficient for an action.
- GET /api/datasets/{id}/forecast?at=ISO runs the fitted first-iteration Ridge adapter for `lims.ht.2.Mg.Sulfur`. `at` is prediction origin and the target is `at+180min`. Telemetry is reconstructed up to origin, LIMS requires sample+240min<=origin. Model selection used 2025 labels, so origins before 2026-01-01 are refused. Missing/OOD applicability gates return status=abstain with null predictions and reasons. Train-only imputation is permitted only within coverage limits. The response includes separate horizon/publication fields, target_time, previous_lab availability, support evidence, leakage check and risk_guard. The forecast is evidence for review; it does not replace the scenario model or authorize an operator action. The same object is embedded in `POST .../decision` as `forecast`.
...
`scenario.predicted_sulfur` equals the last trajectory point at the exact horizon; `steady_state_sulfur` is separate. Hard sulfur cap 10 cannot be relaxed by editable targets. Candidate `objectives` includes throughput/energy/severity proxies and ranking_loss; formula and assumptions are explicit in README. Regime severity uses train-only normalization, not failure probability.

`status=abstain` always has `recommendation=null`, `scenario=null`, `selected_candidate=null`; diagnostic `candidates` and failed safety_gate remain. `basis=scenario_only` means a supplied hypothetical sulfur value, not a data-validated recommendation. Otherwise a usable forecast and fresh unflagged controls/quality are required. `operational_safety_validated=false` and action `review_controls` preserve prototype scope. Missing T95/cetane blocks observed recommendations; scenario_only hypotheses list missing qualities in `unassessed`. Known violations block candidates in both modes. Gate scope is the horizon endpoint and supplied blend, not all transient quality or validated industrial safety.
