...
Сервер не получает исходные большие таблицы автоматически. Их нужно импортировать через UI/API после первого развёртывания или заранее положить в `storage/`. Внешние API в production-контур не требуются; `scikit-learn` нужен только для офлайн-обучения модели.
