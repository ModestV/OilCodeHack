...
|---|---|---|---|
| Quality | snapshot, прогноз | базовая сера (ЛИМС → ПАК → nowcast), прогноз с квантилями риска, T95, нижняя граница цетана с учётом возраста пробы, сравнение анализаторов после калибровки | `backend/agents.py::QualityAgent`, `backend/forecast.py`, `backend/lab_drift.py` |
...
| Optimization | Quality, Reliability, сценарная модель | кандидаты hold/conservative/automatic/экономичные/оптимумы по сетке/смеси, доза присадки для ЦЧ, риск P(S>10) каждого варианта, safety gate, аннотация Парето, выбор | `OptimizationAgent`, `backend/scenarios.py` |
...
|---|---|---|
| `analyser_disagreement` | свежие ПАК и Q21 после калибровки по ЛИМС расходятся > 2 мг/кг (известное смещение Q21 ≈ 1–2 мг/кг конфликтом не считается) | отказ: база серы не подтверждена |
...
| `economy_vs_quality` | экономичные варианты нарушают предел серы или тратят верхнюю половину бюджета риска (P > 0,1) | приоритет качества |
| `stability_vs_quality` | удержание режима недопустимо: P(S > 10) выше порога тревоги или нет запаса 1 мг/кг | самое дешёвое корректирующее изменение (`corrective_action`) или отказ |
| `stability_vs_economy` | выигрыш от изменения меньше `minimum_economic_gain` | удержание режима |

Правила допустимости вариантов (запас по сере, бюджет риска для экономии, предел шага, цетан с присадкой)
описаны в [calculation-rules.md](calculation-rules.md#прогноз-ограничения-и-выбор-варианта).
